...
import com.kxmall.market.data.dto.PermissionPointDTO;
...
import com.kxmall.market.data.mapper.RoleStorageMapper;
...
public class RoleServiceImpl implements RoleService {
    ...
    @Autowired
    private RoleStorageMapper roleStorageMapper;
    ...
    @Override
    public String storagePermissionSet(RoleStorageDTO roleStorageDTO, Long adminId) throws ServiceException {
        Long roleId = roleStorageDTO.getRoleId();
        if (roleId == null) {
            throw new AdminServiceException(ExceptionDefinition.PARAM_CHECK_FAILED);
        }
        roleStorageMapper.delete(new EntityWrapper<RoleStorageDO>().eq("role_id", roleId));
        List<Long> storageIdList = roleStorageDTO.getStorageIdList();
        if (!CollectionUtils.isEmpty(storageIdList)) {
            RoleStorageDO roleStorageDO;
            List<RoleStorageDO> roleStorageDOList = new ArrayList<>();
            for (Long storageId : storageIdList) {
                roleStorageDO = new RoleStorageDO();
                roleStorageDO.setRoleId(roleId);
                roleStorageDO.setStorageId(storageId);
                roleStorageDOList.add(roleStorageDO);
            }
            roleStorageMapper.insertBatch(roleStorageDOList);
        }
        return "ok";
    }

    @Override
    public List<Long> getStoragePermissionSet(Long roleId, Long adminId) throws ServiceException {
        if (roleId == null) {
            throw new AdminServiceException(ExceptionDefinition.PARAM_CHECK_FAILED);
        }
        List<RoleStorageDO> roleStorageDOList = roleStorageMapper.selectList(new EntityWrapper<RoleStorageDO>().eq("role_id", roleId));
        if (roleStorageDOList != null && roleStorageDOList.size() > 0) {
            return roleStorageDOList.stream().map(RoleStorageDO::getStorageId).collect(Collectors.toList());
        }
        return null;
    }
